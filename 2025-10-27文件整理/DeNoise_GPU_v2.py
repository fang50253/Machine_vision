...
class AdvancedDenoiser:
    ...
    def hybrid_denoise_v1(self, image, sharpen_amount=10):
        """混合去噪 + 锐化"""
        # 传统去噪
        traditional_denoised = self.traditional_denoiser.bilateral_denoise(image)
        # 锐化处理
        sharpened_image = self.sharpen_image(final_denoised, sharpen_amount)
        # 深度学习去噪
    ...
    def sharpen_image(self, image, amount=10):
        """
        图像锐化处理
        Args:
        ...
        # 方法1: 使用拉普拉斯算子锐化
        def laplacian_sharpen(img, strength):
            # 创建拉普拉斯滤波器
            ...
            # 调整锐化强度
            strength_factor = strength / 50.0  # 归一化强度
            sharpened = img.astype(np.float32) + laplacian * strength_factor
            
            # 确保数值范围正确
            sharpened = np.clip(sharpened, 0, 255).astype(np.uint8)
            return sharpened
        
        # 方法2: 非锐化掩模 (Unsharp Mask)
        def unsharp_mask_sharpen(img, strength):
            # 高斯模糊
            ...
            sharpened = img.astype(np.float32) + mask * strength_factor
        ...
        # 方法3: 自定义锐化核
        def custom_sharpen(img, strength):
            # 创建锐化核
            ...
            # 调整核的强度
            base_strength = 8.0
            adjust_factor = strength / 50.0
            sharpen_kernel[1, 1] = base_strength + adjust_factor
            
            # 应用锐化
            sharpened = cv2.filter2D(img.astype(np.float32), -1, sharpen_kernel)
        ...
        if len(image.shape) == 2:
            # 灰度图像 - 使用非锐化掩模
            sharpened = unsharp_mask_sharpen(image, amount)
        else:
            # 彩色图像 - 对每个通道分别处理
            sharpened_channels = []
            for i in range(3):
                channel = image[:, :, i]
                # 使用拉普拉斯锐化保持边缘
                sharpened_channel = laplacian_sharpen(channel, amount)
    ...
    def adaptive_sharpen(self, image, base_amount=10, edge_boost=True):
        """
        自适应锐化 - 根据图像内容调整锐化强度
        Args:
            image: 输入图像
            base_amount: 基础锐化强度
            edge_boost: 是否在边缘区域增强锐化
        """
        if base_amount <= 0:
            return image
        
        # 检测边缘区域
        if edge_boost and len(image.shape) == 2:
            # 灰度图像的边缘检测
        ...
        # 基础锐化
        base_sharpened = self.sharpen_image(image, base_amount)
        
        if edge_boost and edge_mask is not None:
            # 在边缘区域应用更强的锐化
            enhanced_amount = min(base_amount * 1.5, 30)  # 限制最大强度
            edge_enhanced = self.sharpen_image(image, enhanced_amount)
            
            # 混合图像
            result = base_sharpened.copy()
            if len(image.shape) == 2:
                result[edge_mask] = edge_enhanced[edge_mask]
            else:
                for i in range(3):
                    result[:, :, i][edge_mask] = edge_enhanced[:, :, i][edge_mask]
            
            return result
        else:
            return base_sharpened

    def hybrid_denoise_v2(self, image, sharpen_amount=10, use_adaptive_sharpen=True):
        """
        增强版混合去噪
        Args:
            image: 输入图像
            sharpen_amount: 锐化强度 (0-30推荐范围)
            use_adaptive_sharpen: 是否使用自适应锐化
        """
        # 传统去噪
        traditional_denoised = self.traditional_denoiser.bilateral_denoise(image)
        ...
        if use_adaptive_sharpen:
            sharpened_image = self.adaptive_sharpen(final_denoised, sharpen_amount)
        else:
            sharpened_image = self.sharpen_image(final_denoised, sharpen_amount)
    ...
    # 在类中添加锐化强度验证方法
    def validate_sharpen_amount(self, amount):
        """验证锐化强度参数"""
        if amount < 0:
            return 0
        elif amount > 50:  # 设置最大限制防止过度锐化
            print(f"警告: 锐化强度 {amount} 过高，已限制为 50")
            return 50
        else:
            return amount
    ...
    def hybrid_denoise_v1(self, image, sharpen_amount=10):
        """混合去噪 + 锐化 (带参数验证)"""
        # 验证锐化强度
        validated_amount = self.validate_sharpen_amount(sharpen_amount)
        ...
        if validated_amount > 0:
            sharpened_image = self.sharpen_image(final_denoised, validated_amount)
            return sharpened_image
        else:
...
def process_single_image():
    """处理单张图像"""
    from Denoiser2 import get_image_path, get_model_path, get_image_size_choice, load_and_process_image
    from Denoiser2 import denoise_program
