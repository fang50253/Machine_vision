...
def main():
    ...
    # 配置参数
    config = {
        'results_dir': 'batch_results_20251222_170024/images',  # 你的结果目录
        'batch_size': 8,
        'num_epochs': 50,
        'learning_rate': 1e-4,
        'patch_size': 128,
        'val_ratio': 0.2,  # 20%作为验证集
        'save_dir': 'checkpoints/dncnn_to_original',
        'visualization_dir': 'dncnn_to_original_viz'
    }
    ...
    for epoch in range(1, config['num_epochs'] + 1):
        ...
        if epoch % 5 == 0:
            ...
            plt.subplot(1, 3, 2)
            plt.plot(train_history['recon'], label='Train')
            plt.plot(val_history['recon'], label='Val')
            ...
            plt.subplot(1, 3, 3)
            plt.plot(train_history['edge'], label='Train')
            plt.plot(val_history['edge'], label='Val')
            plt.title('Edge Loss')
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.legend()
