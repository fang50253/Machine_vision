...
def single_image_processing():
    ...
    try:
        ...
        # 边缘增强模型默认路径
        default_edge_model = os.path.join('trained_models', 'dncnn_to_original', 'best_model.pth')
        print(f"\n默认边缘增强模型: {default_edge_model}")
        edge_input = input("请输入边缘增强模型路径，回车使用默认（或输入 none 跳过）: ").strip().strip('\"\'')
        if edge_input.lower() in ('none', 'no', 'n'):
            edge_model_path = None
            print("跳过边缘增强模型（会回退为 DnCNN 结果）")
        elif edge_input == '':
            edge_model_path = default_edge_model
            if not os.path.exists(edge_model_path):
                print(f"警告: 默认边缘增强模型不存在: {edge_model_path}，后续将尝试加载并在失败时回退")
        else:
            edge_model_path = edge_input
            if not os.path.exists(edge_model_path):
                print(f"警告: 提供的边缘增强模型不存在: {edge_model_path}，后续将尝试加载并在失败时回退")
        
        # 初始化控制器
        controller = DenoiseController()
        # 传入两个模型路径：去噪模型与边缘增强模型
        if not controller.initialize_denoiser(model_path, edge_model_path):
            print("警告: 去噪器初始化失败")
            return
...
def batch_processing():
    ...
    try:
        ...
        # 传入边缘增强模型路径（BatchController 已兼容两个参数）
        if not controller.initialize_denoiser(model_path, edge_model_path):
            print("警告: 去噪器初始化失败")
            return
